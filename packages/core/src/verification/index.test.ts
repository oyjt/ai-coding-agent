...
test('S 级任务只要求 lint', () => {
  ...
  try {
    ...
    const plan = createVerificationPlan({ cwd, taskLevel: 'S', projectType: 'vue' });
...
test('M 级任务要求 lint、test、typecheck', () => {
  ...
  try {
    ...
    const plan = createVerificationPlan({ cwd, taskLevel: 'M', projectType: 'react' });
...
test('L 级任务增加 build，并记录回滚审查门禁', () => {
  ...
  try {
    writeFileSync(join(cwd, 'package.json'), JSON.stringify({ scripts: { lint: 'echo lint', test: 'echo test', typecheck: 'echo typecheck', build: 'echo build' } }));
    writeFileSync(join(cwd, 'pnpm-lock.yaml'), 'lockfileVersion: 9.0\n');
    const plan = createVerificationPlan({ cwd, taskLevel: 'L', projectType: 'react-native' });
...
test('缺失验证脚本会进入 manual gates，而不是伪造通过', () => {
  ...
  try {
    ...
    writeFileSync(join(cwd, 'pnpm-lock.yaml'), 'lockfileVersion: 9.0\n');
    const plan = createVerificationPlan({ cwd, taskLevel: 'M', projectType: 'node' });
    assert.deepEqual(plan.commands.map((item) => item.id), ['lint']);
